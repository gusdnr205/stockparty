...
price_fields = {
    ...
    "diff":fields.Float,
    "volume":fields.Float,
    "diff_vol":fields.Float,
    "chdegree":fields.Float,
    "sojinrate":fields.Float,
    "changerate":fields.Float,
    "fpvolume":fields.Float,
    "covolume":fields.Float,
    "value":fields.Float,
    "ppvolume":fields.Float,
...
class Price(Resource):
    ...
    def get(self, code):
        ...
        default_start_date = datetime.datetime.now() - datetime.timedelta(days=14)
        start_date = request.args.get('start_date', default=default_start_date.strftime("%Y%m%d"), type=str)
        start_date2 ="19971030"
        end_date = request.args.get('end_date', default=today, type=str)
        results = list(mongodb.find_items({"code":code, "날짜": {"$gte":start_date2, "$lte":end_date}}, 
                                            "stocklab", "price_info"))
        result_object = {}
        price_info_list = []
        for item in results:
...
class Price2(Resource):
    @marshal_with(price_list_fields)
    def get(self, code):
        today = datetime.datetime.now().strftime("%Y%m%d")
        default_start_date = datetime.datetime.now() - datetime.timedelta(days=2)
        start_date = request.args.get('start_date', default=default_start_date.strftime("%Y%m%d"), type=str)
        start_date2="20201129"
        end_date = request.args.get('end_date', default=today, type=str)
        results = list(mongodb.find_items({"code":code, "날짜": {"$gte":start_date2, "$lte":end_date}}, 
                                            "stocklab", "price_info"))
        result_object = {}
...
api.add_resource(Price2, "/codes/<string:code>/price2", endpoint="price2")
