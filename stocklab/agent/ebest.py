...
class EBest:
    ...
    def login(self):
        ...
        self.xa_session_client.Login(self.user, self.passwd,0, 0, 0)
    ...
    def _execute_query2(self, res, in_block_name, out_block_name, *out_fields, **set_fields):
        
        self.day = ""
        time.sleep(1) #api 한계를 이유로 넣어줌 
        print("current query cnt:", len(self.query_cnt)) #현재 쿼리 개수 
        print(res, in_block_name, out_block_name)# 현재  res 경로, trcode inblock 이름,trcode outblock 이름 
        while len(self.query_cnt) >= EBest.QUERY_LIMIT_10MIN: # 분당 개수 제한있어서 분당 최대개수보다 크면 1초마다 현재개수와함께 waiting 출력
            time.sleep(1)
        ...
        xa_query = win32com.client.DispatchWithEvents("XA_DataSet.XAQuery", XAQuery) # 연결
        ...
        if self.day != "": # self.ctstime 값이 존재하면, 연속 data 가 있다는 의미...
            time.sleep(0.5) # TR 횟수 제한때문에...
            self.continue_search(self.day)
    ...
    def get_current_call_price_by_code(self, code=None):
        ...
        return result
    def get_stock_price_by_code2(self, code=None ,cnt="1"):
        ...
        result = self._execute_query2("t1305", 
                                "t1305InBlock", 
                                "t1305OutBlock1",
                                *out_params,
                                **in_params)
        ...
        return result
    def continue_search(self, day):
        """
        연속조회하기
        """
        print("-----------------------------------------")
        self.is_data_received = False
        self.SetFieldData("t1305InBlock", "date", 0, day)
    ...
    def get_stock_price_by_code(self, code=None ,cnt="1"):
        ...
        result = self._execute_query("t1305", 
                                ...
                                *out_params,
                                **in_params)
        pandf =pd.read_csv("C:/Users/GODSBEER/Desktop/lstm/final1.csv",encoding='utf-8')
